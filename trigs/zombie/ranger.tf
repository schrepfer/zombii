...
;; $HeadURL: file:///storage/subversion/projects/ZombiiTF/zombii/trigs/zombie/ranger.tf $
;;
/eval /loaded $[substr('$HeadURL: file:///storage/subversion/projects/ZombiiTF/zombii/trigs/zombie/ranger.tf $', 10, -2)]
...
/def -Fp5 -msimple -h'SEND @on_enter_game' on_enter_game_ranger = \
  /unset ranger_pet_name%; \
  /set ranger_progress_axe_fighting_techniques_last=%{ranger_progress_axe_fighting_techniques}%; \
  /set ranger_progress_sword_fighting_techniques_last=%{ranger_progress_sword_fighting_techniques}%; \
  /set ranger_progress_archery_last=%{ranger_progress_archery}%; \
  /set ranger_progress_wilderness_survival_last=%{ranger_progress_wilderness_survival}%; \
  /set ranger_progress_beastmastery_last=%{ranger_progress_beastmastery}
...
/def -Fp5 -mregexp -t'^You will advance in (axe fighting techniques|sword fighting techniques|archery|wilderness survival|beastmastery) (.+)\\.$' path_progress = \
  ...
  /else \
    /set _progress=0%; \
  /endif%; \
  /let _key=ranger_progress_$[replace(' ', '_', {P1})]%; \
  /let _last=%{_key}_last%; \
  /let _delta=$[_progress - expr(_last)]%; \
  /while (_delta < 0) \
    /test _delta += 10%; \
  /done%; \
...
/def -Fp5 -msimple -h'SEND @save' save_ranger = /mapcar /listvar ranger_pet_* ranger_progress_*_last %| /writefile $[save_dir('ranger')]
/eval /load $[save_dir('ranger')]

/test ranger_progress_axe_fighting_techniques_last := ranger_progress_axe_fighting_techniques_last ? ranger_progress_axe_fighting_techniques_last : ranger_progress_axe_fighting_techniques
/test ranger_progress_sword_fighting_techniques_last := ranger_progress_sword_fighting_techniques_last ? ranger_progress_sword_fighting_techniques_last : ranger_progress_sword_fighting_techniques
/test ranger_progress_archery_last := ranger_progress_archery_last ? ranger_progress_archery_last : ranger_progress_archery
/test ranger_progress_wilderness_survival_last := ranger_progress_wilderness_survival_last ? ranger_progress_wilderness_survival_last : ranger_progress_wilderness_survival
/test ranger_progress_beastmastery_last := ranger_progress_beastmastery_last ? ranger_progress_beastmastery_last : ranger_progress_beastmastery
