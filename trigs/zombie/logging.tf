...
/def start_log = \
  ...
  /endif%; \
  /test log_stream := tfopen(logs_dir('messages'), 'w')%; \
  /log_hour
...
/def -Fp10 -mregexp -t'^(([A-Z][a-z0-9-]+) (the toasted |\(helper\) )?| |)?([\\[{<]([a-z]+)[>}\\]]:) (.*)$' log_channel = \
  ...
  /else \
    /let _color=cyan%; \
  /endif%; \
  /if ({P5} =~ 'party' & {P6} =/ 'hp: * (*) sp: * (*)*') \
    /return%; \
  /endif%; \
  /if (strlen(log_channels) & !isin({P5}, log_channels)) \
    /return%; \
  /endif%; \
  /if (is_me({P2}) & !log_self) \
    /return%; \
  /endif%; \
...
/def -Fp10 -mregexp -t'^(Ghost of )?([A-Za-z][a-z]+) tells you' log_tell = \
