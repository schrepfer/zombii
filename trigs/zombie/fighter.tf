...
/test fighter_rounds := fighter_rounds ? fighter_rounds : 0
/test fighter_last := fighter_last ? fighter_last : 0

/def -Fp5 -msimple -h'SEND @on_new_round' on_new_round_fighter = \
  /test fighter_rounds += 1
...
/def -Fp5 -msimple -t'You gained an offensive point!' gained_offensive_point = \
  ...
  !2 pigeon whee%; \
  /let _time=$[time()]%; \
  /say -c'red' -- Gained Fighter OFFENSIVE Point ($[pluralize(fighter_rounds, 'round')], $[to_dhms(_time - (fighter_last ? fighter_last : _time + 1))])!%; \
  /set fighter_rounds=0%; \
  /test fighter_last := time()
...
/def -Fp5 -msimple -t'You gained a defensive point!' gained_defensive_point = \
  ...
  !2 pigeon whee%; \
  /let _time=$[time()]%; \
  /say -c'red' -- Gained Fighter DEFENSIVE Point ($[pluralize(fighter_rounds, 'round')], $[to_dhms(_time - (fighter_last ? fighter_last : _time + 1))])!%; \
  /set fighter_rounds=0%; \
  /test fighter_last := time()
...
/def -Fp5 -mregexp -t'^ Overwhelming strikes            (\d+)' fig_skills_show_overwhelming_strikes = \
  /set fighter_mastery_overwhelming_strikes=%{P1}
...
/def -Fp5 -mregexp -t'^ Unshakable                      (\d+)' fig_skills_show_unshakable = \
  /set fighter_mastery_unshakable=%{P1}
...
/def -Fp5 -msimple -h'SEND @save' save_fighter = \
  /mapcar /listvar fighter_mastery_* fighter_points fighter_last fighter_rounds %| /writefile $[save_dir('fighter')]%; \
